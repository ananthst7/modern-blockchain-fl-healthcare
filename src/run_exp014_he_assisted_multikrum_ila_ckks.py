...
This experiment makes CKKS functional in the model-update path.
...
DEFAULT_OUTPUT_DIR = Path("results/he_assisted_multikrum_ila_ckks")
...
def run_experiment(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    ledger = PoALedger(
        ...
        genesis_metadata={
            ...
            "experiment_name": "HE-Assisted Multi-Krum with Functional ILA-CKKS Aggregation",
            "architecture": "EfficientNet-B0 + FedDyn + HE-assisted Multi-Krum + functional ILA-CKKS + PoA blockchain",
            "seed": args.seed,
            "data_policy": "No raw patient data, raw images, or plaintext selected client updates are stored on-chain.",
        },
    )
    ...
    for round_idx in range(1, args.global_rounds + 1):
        ...
        poa_block_time_ms = create_round_block(
            ...
            metrics=metrics,
            algorithm="FedDyn + HE-assisted Multi-Krum + functional ILA-CKKS",
        )
        ...
        row = {
            ...
            **distance_metrics,
            ...
            "functional_he_update_scope": "ILA-selected trainable tensors; non-selected floating buffers are synchronized from the same Multi-Krum-selected clients",
            "server_plaintext_selected_update_access": False,
            "trusted_authority_outputs_decrypted": ["pairwise distance scores", "final selected-tensor aggregate"],
            "known_leakage": ["pairwise distance scores", "selected client indices", "selected tensor names and sizes", "final selected-tensor aggregate"],
        ...
        print(
            ...
            f"HE-MK selected: {selected_indices} | PER: {row.get('avg_parameter_encryption_ratio', 0):.4f} | "
    ...
    final_results = {
        ...
        "he_library": "TenSEAL",
        "he_scheme": "CKKS",
        "selection_strategy": "client-side Information Leakage-Aware metadata selection",
        "max_selected_bytes": args.max_selected_bytes,
        "functional_he_update": True,
        "functional_he_update_scope": "ILA-selected trainable tensors with selected-client buffer synchronization",
        "blockchain_enabled": True,
        "privacy_architecture_note": "Server role does not receive plaintext selected client updates; trusted authority decrypts distance scores and final selected-tensor aggregate. Single-process prototype still simulates all roles in one runtime.",
        "malicious_client_index": args.malicious_client_index,
        "attack_scale": args.attack_scale,
        "seed": args.seed,
        "device": str(device),
    ...
    save_json(output_dir / "covid_he_assisted_multikrum_ila_ckks.json", final_results)
    write_round_history_csv(output_dir, history)

    if args.save_model:
        torch.save(global_model.state_dict(), output_dir / "covid_he_assisted_multikrum_ila_ckks.pth")
    ...
    print("\nSaved EXP-014 outputs to:", output_dir)
    return final_results


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="EXP-014 HE-assisted Multi-Krum + functional ILA-CKKS")
