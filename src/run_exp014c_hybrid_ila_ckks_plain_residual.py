...
This experiment implements the practical selective-encryption path:

1. Clients train the full EfficientNet-B0/FedDyn model.
2. ILA selects the highest leakage-risk trainable tensors under a byte budget.
3. Only ILA-selected sensitive tensors are CKKS-encrypted.
4. HE-derived distances over selected sensitive tensors drive Multi-Krum client selection.
5. Selected sensitive tensors are homomorphically averaged and decrypted by the trusted-authority role.
6. Non-selected lower-risk residual trainable tensors are aggregated in plaintext from the same Multi-Krum-selected clients.
7. The global model is rebuilt by merging HE-sensitive tensors + plaintext residual tensors + selected-client buffers.
...
simulated in one script. The server role is treated as receiving plaintext only
for the non-selected residual tensors, not for the ILA-selected sensitive tensors.
"""
...
DEFAULT_OUTPUT_DIR = Path("results/exp014c_hybrid_ila_ckks_plain_residual")
...
def get_trainable_floating_keys(state: Dict[str, torch.Tensor]) -> List[str]:
    return [
        key
        for key, value in state.items()
        if torch.is_floating_point(value) and is_trainable_tensor_key(key)
    ]


def compute_hybrid_scope_metrics(
    global_state: Dict[str, torch.Tensor],
    selected_keys: Sequence[str],
) -> Dict[str, Any]:
    selected_key_set = set(selected_keys)
    trainable_keys = get_trainable_floating_keys(global_state)
    residual_keys = [key for key in trainable_keys if key not in selected_key_set]

    selected_values = sum(int(global_state[key].numel()) for key in selected_keys if key in global_state)
    residual_values = sum(int(global_state[key].numel()) for key in residual_keys if key in global_state)
    total_values = selected_values + residual_values

    selected_bytes = sum(int(global_state[key].numel() * global_state[key].element_size()) for key in selected_keys if key in global_state)
    residual_bytes = sum(int(global_state[key].numel() * global_state[key].element_size()) for key in residual_keys if key in global_state)
    total_bytes = selected_bytes + residual_bytes

    return {
        "hybrid_selected_sensitive_key_count": len(selected_keys),
        "hybrid_plaintext_residual_key_count": len(residual_keys),
        "hybrid_total_trainable_key_count": len(trainable_keys),
        "hybrid_selected_sensitive_values": selected_values,
        "hybrid_plaintext_residual_values": residual_values,
        "hybrid_total_trainable_values": total_values,
        "hybrid_selected_sensitive_bytes": selected_bytes,
        "hybrid_plaintext_residual_bytes": residual_bytes,
        "hybrid_total_trainable_bytes": total_bytes,
        "hybrid_sensitive_he_parameter_ratio": selected_values / max(1, total_values),
        "hybrid_plaintext_residual_parameter_ratio": residual_values / max(1, total_values),
        "hybrid_plaintext_residual_keys": residual_keys,
    }


def merge_hybrid_he_sensitive_with_plaintext_residual(
    global_state: Dict[str, torch.Tensor],
    ...
    """
    Build the next global state for EXP-014C.

    - ILA-selected sensitive trainable tensors come from the decrypted CKKS
      aggregate.
    - Non-selected residual trainable tensors are averaged in plaintext from
      the same Multi-Krum-selected clients.
    - Floating buffers, especially BatchNorm running_mean/running_var, are also
      synchronized from the same selected clients.
    - Non-floating buffers are copied from the first selected client.

    This is intentionally a hybrid privacy/utility design, not a full-HE claim.
    """
    ...
    # 1) Insert HE aggregate for sensitive ILA-selected trainable tensors.
    for key in selected_key_set:
        if key in decrypted_selected_state:
            merged[key] = decrypted_selected_state[key].detach().cpu().clone()

    # 2) Average every non-selected floating tensor in plaintext from the same
    # robust clients. This includes both residual trainable parameters and
    ...
    for key in list(merged.keys()):
        ...
        if torch.is_floating_point(merged[key]):
            avg = torch.zeros_like(merged[key].detach().cpu())
            found_any = False
            for client_state, size in zip(selected_client_states, selected_client_sizes):
                if key in client_state and torch.is_floating_point(client_state[key]):
                    avg += client_state[key].detach().cpu() * (float(size) / total_samples)
                    found_any = True
            if found_any:
                merged[key] = avg
        else:
...
def create_round_block(
    ...
    aggregation_hash = sha256_hex({"round": round_idx, "selected_clients": selected_clients, "operation": "Hybrid ILA-CKKS sensitive-tensor HE aggregation + plaintext residual averaging", "algorithm": algorithm})
    global_model_hash = sha256_hex({"round": round_idx, "accuracy": metrics.get("accuracy"), "f1": metrics.get("f1"), "note": "global model after hybrid HE-sensitive + plaintext-residual aggregation"})
    transactions.append(
        make_aggregation_transaction(
            round_number=round_idx,
...
def write_markdown_report(output_dir: Path, results: Dict[str, Any], blockchain_summary: Dict[str, Any]) -> None:
    ...
    lines: List[str] = []
    lines.append("# EXP-014C — Hybrid ILA-CKKS + Plaintext Residual Multi-Krum\n")
    lines.append("## Purpose\n")
    lines.append("EXP-014C implements a practical selective-encryption workflow: ILA-selected high-leakage tensors are aggregated through CKKS, while non-selected lower-risk residual tensors are aggregated in plaintext from the same Multi-Krum-selected clients.\n")
    lines.append("## Architecture\n")
    lines.append("```text")
    lines.append("EfficientNet-B0 + FedDyn + ILA sensitive-tensor selection + CKKS selected tensors")
    ...
    lines.append("+ plaintext residual aggregation + full global model update + PoA blockchain")
    lines.append("```\n")
    ...
    lines.append("\n## Valid Claims\n")
    lines.append("- CKKS is used for the functional ILA-selected sensitive-tensor update path.")
    ...
    lines.append("- Selected sensitive tensors are homomorphically averaged.")
    lines.append("- The decrypted HE sensitive-tensor aggregate is merged with plaintext residual aggregation to update the full global model.")
    lines.append("- Non-selected residual trainable tensors and BatchNorm/non-trainable buffers are aggregated/synchronized from the same Multi-Krum-selected clients.")
    lines.append("- The PoA ledger records hashes, selected clients, metrics and rewards.\n")
    lines.append("## Claims to Avoid\n")
    lines.append("- Fully encrypted aggregation of every trainable tensor.")
    lines.append("- Claiming that the non-selected residual tensors are hidden from the aggregation server.")
    lines.append("- Removal of the trusted-authority assumption.")
    lines.append("- Formal MPC-level privacy.\n")
    (output_dir / "EXP-014C_HYBRID_ILA_CKKS_PLAINTEXT_RESIDUAL_RESULTS.md").write_text("\n".join(lines), encoding="utf-8")


def run_single_budget(args: argparse.Namespace) -> Dict[str, Any]:
    set_seed(args.seed)
    ...
    ledger = PoALedger(
        ...
        genesis_metadata={
            "experiment_id": "EXP-014C",
            "experiment_name": "Hybrid ILA-CKKS + Plaintext Residual Multi-Krum",
            "architecture": "EfficientNet-B0 + FedDyn + ILA sensitive HE aggregation + plaintext residual aggregation + PoA blockchain",
            "seed": args.seed,
            "data_policy": "No raw patient data, raw images, or raw tensor values are stored on-chain. Plaintext residual aggregation occurs off-chain in the server role.",
        },
    )
    ...
    for round_idx in range(1, args.global_rounds + 1):
        print(f"\n===== EXP-014 HE-Assisted Multi-Krum Round {round_idx}/{args.global_rounds} =====")
        round_start = time.time()

        old_global_state_cpu = {k: v.detach().cpu().clone() for k, v in global_model.state_dict().items()}
        old_global_trainable_cpu = {k: v.detach().cpu().clone() for k, v in global_model.state_dict().items() if torch.is_floating_point(v)}
        ...
        for client_idx, client_dir in enumerate(client_dirs):
            ...
            if client_idx == args.malicious_client_index:
                ...
                state = sign_flip_attack(state, scale=args.attack_scale)
            ...
            h_states[client_idx] = update_h_state(
                h_state=h_states[client_idx],
                client_state=state,
                global_state=old_global_trainable_cpu,
                alpha=args.alpha,
            )

        selected_keys, selected_plain_budget_bytes, ranked_ila_scores = select_ila_keys_from_metadata(
            client_score_rows_by_client=client_score_rows_by_client,
            max_selected_bytes=args.max_selected_bytes,
        )
        print(f"Selected {len(selected_keys)} ILA keys | plaintext bytes={selected_plain_budget_bytes}")
        ...
        hybrid_scope_metrics = compute_hybrid_scope_metrics(old_global_state_cpu, selected_keys)

        new_global_state = merge_hybrid_he_sensitive_with_plaintext_residual(
            global_state=old_global_state_cpu,
        ...
        fixed_keys = [k for k in old_global_state_cpu if "classifier" in k and torch.is_floating_point(old_global_state_cpu[k])]
        adaptive_rows = []
        ila_rows = []
        independent_rows = []
        ...
        crypto_time = total_encryption_time + float(distance_metrics["he_distance_time_sec"]) + float(encrypted_aggregation_time) + float(decryption_time)
        ...
        poa_block_time_ms = create_round_block(
            ...
            metrics=metrics,
            algorithm="FedDyn + Hybrid ILA-CKKS sensitive aggregation + plaintext residual",
        )
        ...
        row = {
            ...
            **distance_metrics,
            ...
            "selected_key_count": len(selected_keys),
            "selected_keys": selected_keys,
            "selection_strategy": "client-side ILA metadata; score = update_norm * fisher_score * gradient_variance",
            ...
            **{k: v for k, v in hybrid_scope_metrics.items() if k != "hybrid_plaintext_residual_keys"},
            "hybrid_plaintext_residual_keys_sample": hybrid_scope_metrics.get("hybrid_plaintext_residual_keys", [])[:40],
            "hybrid_update": True,
            "hybrid_update_scope": "ILA-selected high-leakage tensors use CKKS/HE aggregation; non-selected lower-risk residual tensors use plaintext aggregation from the same Multi-Krum-selected clients.",
            "functional_he_update": True,
            "functional_he_update_scope": "ILA-selected high-leakage trainable tensors are HE-aggregated; non-selected residual trainable tensors are plaintext-aggregated from same selected clients",
            "server_plaintext_selected_update_access": False,
            "server_plaintext_residual_update_access": True,
            "trusted_authority_outputs_decrypted": ["pairwise distance scores", "final selected sensitive-tensor aggregate"],
            "known_leakage": ["pairwise distance scores", "selected client indices", "selected sensitive tensor names and sizes", "final selected sensitive-tensor aggregate", "plaintext residual tensor values"],
        ...
        print(
            ...
            f"HE-MK selected: {selected_indices} | Sensitive HE PER: {row.get('avg_parameter_encryption_ratio', 0):.4f} | "
    ...
    blockchain_summary = {
        ...
        "data_policy": "Only hashes, metrics, selected-client metadata, distance-result metadata and reward records are stored on-chain.",
    }
    ...
    final_results = {
        "experiment": "EXP-014C",
        "method": "Hybrid ILA-CKKS Sensitive Tensor Aggregation + Plaintext Residual Multi-Krum",
        ...
        "aggregation": "HE-assisted Multi-Krum over ILA-selected sensitive tensors with plaintext residual aggregation",
        "he_library": "TenSEAL",
        "he_scheme": "CKKS",
        "selection_strategy": "client-side Information Leakage-Aware metadata selection for sensitive HE path",
        "max_selected_bytes": args.max_selected_bytes,
        "functional_he_update": True,
        "functional_he_update_scope": "ILA-selected sensitive trainable tensors use HE aggregation; non-selected residual trainable tensors use plaintext aggregation",
        "blockchain_enabled": True,
        "privacy_architecture_note": "Server role does not receive plaintext ILA-selected sensitive updates, but does receive plaintext non-selected residual updates. Trusted authority decrypts distance scores and final selected sensitive-tensor aggregate. Single-process prototype simulates all roles in one runtime.",
        "malicious_client_index": args.malicious_client_index,
        "attack_scale": args.attack_scale,
        "seed": args.seed,
        "device": str(device),
        "total_time_sec": total_time,
        "hybrid_design": "ILA-selected sensitive tensors are HE-aggregated; lower-risk residual tensors are plaintext-aggregated from same Multi-Krum-selected clients.",
        "history": history,
        "final_metrics": history[-1],
        "best_metrics": max(history, key=lambda row: row["accuracy"]),
        "blockchain_summary": blockchain_summary,
    }
    save_json(output_dir / f"covid_exp014c_hybrid_ila_ckks_plain_residual_budget_{args.max_selected_bytes}.json", final_results)
    write_round_history_csv(output_dir, history)

    if args.save_model:
        torch.save(global_model.state_dict(), output_dir / f"covid_exp014c_hybrid_ila_ckks_plain_residual_budget_{args.max_selected_bytes}.pth")

    write_markdown_report(output_dir, final_results, blockchain_summary)

    print("\n===== EXP-014C Hybrid Budget Summary =====")
    print(json.dumps({
    ...
    print("\nSaved EXP-014C budget outputs to:", output_dir)
    return final_results



def write_exp014c_master_summary(output_dir: Path, results: Sequence[Dict[str, Any]]) -> None:
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    for result in results:
        ...
        rows.append({
            ...
            "selected_key_count": final.get("selected_key_count", 0),
            "sensitive_he_parameter_ratio": final.get("hybrid_sensitive_he_parameter_ratio", final.get("avg_parameter_encryption_ratio", 0.0)),
            "plaintext_residual_parameter_ratio": final.get("hybrid_plaintext_residual_parameter_ratio", 0.0),
            ...
            "final_plain_selected_upload_mb": final.get("selective_plain_upload_mb", 0.0),
    ...
    save_json(output_dir / "exp014c_hybrid_summary.json", {"experiments": rows})

    if rows:
        with (output_dir / "exp014c_hybrid_summary.csv").open("w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(f, fieldnames=list(rows[0].keys()))
            writer.writeheader()
            writer.writerows(rows)

    lines: List[str] = []
    lines.append("# EXP-014C Hybrid ILA-CKKS + Plaintext Residual Summary\n")
    lines.append("EXP-014C evaluates a practical selective-encryption workflow: ILA-selected high-leakage tensors are CKKS/HE aggregated, while non-selected lower-risk residual tensors are plaintext-aggregated from the same Multi-Krum-selected clients.\n")
    lines.append("| Budget | HE-sensitive Keys | HE-sensitive PER | Plaintext Residual PER | Final Acc | Final F1 | Best Acc | Encrypted Upload | Crypto Overhead | Chain Valid | Tamper |")
    lines.append("|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|---|")
    for row in rows:
        lines.append(
            f"| {row['budget_bytes']:,} | {row['selected_key_count']} | "
            f"{row['sensitive_he_parameter_ratio'] * 100:.2f}% | {row['plaintext_residual_parameter_ratio'] * 100:.2f}% | "
            f"{row['final_accuracy'] * 100:.2f}% | {row['final_f1'] * 100:.2f}% | "
            f"{row['best_accuracy'] * 100:.2f}% | {row['final_encrypted_upload_mb']:.2f} MB | "
            f"{row['final_crypto_overhead_percent']:.2f}% | {row['chain_valid']} | {row['tamper_detected']} |"
        )
    lines.append("\n## Correct Interpretation\n")
    lines.append("EXP-014C is not a full-HE experiment. It is a hybrid privacy-utility experiment. The sensitive selected tensors are protected through CKKS and HE aggregation; the residual tensors are intentionally exposed to preserve utility and reduce encryption cost.\n")
    (output_dir / "EXP-014C_HYBRID_MASTER_SUMMARY.md").write_text("\n".join(lines), encoding="utf-8")
...
def run_experiment(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    for budget in args.budgets:
        ...
        budget_args.output_dir = str(master_output_dir / f"budget_{int(budget)}")
        results.append(run_single_budget(budget_args))
    write_exp014c_master_summary(master_output_dir, results)
    return {"experiment": "EXP-014C", "budget_results": results}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="EXP-014C hybrid ILA-CKKS sensitive aggregation + plaintext residual Multi-Krum")
    ...
    parser.add_argument("--max-selected-bytes", type=int, default=2_000_000, help="Single-budget compatibility value; --budgets controls normal sweeps.")
