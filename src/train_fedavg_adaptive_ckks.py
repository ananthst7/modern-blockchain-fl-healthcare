...
RESULTS_DIR = Path("results/fedavg_adaptive_ckks")
...
def merge_selected_into_global(global_state, selected_state, selected_keys):
    merged_state = {
    ...
    for key in selected_keys:
        if key in selected_state and torch.is_floating_point(selected_state[key]):
            merged_state[key] = selected_state[key].detach().cpu().clone()
...
def main():
    ...
    for round_idx in range(1, GLOBAL_ROUNDS + 1):
        print(f"\n===== Adaptive CKKS FedAvg Round {round_idx}/{GLOBAL_ROUNDS} =====")
        ...
        new_global_state = merge_selected_into_global(
            global_state=plain_avg_state,
            selected_state=decrypted_selected_state,
            selected_keys=selected_keys,
        )
    ...
    final_results = {
        "experiment": "EXP-007D",
        "method": "Adaptive Selective CKKS Encrypted Aggregation using TenSEAL",
        ...
        "he_library": "TenSEAL",
        "he_scheme": "CKKS",
        "selection_strategy": "top-k tensors by update norm",
        "adaptive_top_k": TOP_K,
        "ckks_poly_modulus_degree": CKKS_POLY_MODULUS_DEGREE,
        "ckks_coeff_mod_bit_sizes": CKKS_COEFF_MOD_BIT_SIZES,
        "ckks_global_scale": CKKS_GLOBAL_SCALE,
    ...
    with open(RESULTS_DIR / "covid_fedavg_adaptive_ckks.json", "w") as f:
        json.dump(final_results, f, indent=4)

    torch.save(
        global_model.state_dict(),
        RESULTS_DIR / "covid_fedavg_adaptive_ckks.pth",
    )

    print("\nSaved Adaptive CKKS FedAvg results to:", RESULTS_DIR)
