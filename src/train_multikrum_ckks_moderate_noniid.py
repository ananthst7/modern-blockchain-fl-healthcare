...
RESULTS_DIR = Path("results/multikrum_ckks_moderate_noniid")
...
def main():
    ...
    for round_idx in range(1, GLOBAL_ROUNDS + 1):
        print(f"\n===== Multi-Krum + Selective CKKS Round {round_idx}/{GLOBAL_ROUNDS} =====")
        ...
        # CKKS aggregation is profiled for encrypted classifier transmission/aggregation.
        ...
        # Correct robust aggregation remains Multi-Krum.
        ...
        row = {
            ...
            "aggregation_note": "Multi-Krum aggregation is kept unchanged; CKKS classifier aggregation is profiled.",
    ...
    final_results = {
        "experiment": "EXP-007C",
        "method": "Multi-Krum + Selective CKKS Profiling using TenSEAL",
    ...
    with open(RESULTS_DIR / "covid_multikrum_ckks_moderate_noniid.json", "w") as f:
        json.dump(final_results, f, indent=4)

    torch.save(
        global_model.state_dict(),
        RESULTS_DIR / "covid_multikrum_ckks_moderate_noniid.pth",
    )

    print("\nSaved Multi-Krum + CKKS results to:", RESULTS_DIR)
