...
RESULTS_DIR = Path("results/feddyn_ckks_profiled")
...
def main():
    ...
    for round_idx in range(1, GLOBAL_ROUNDS + 1):
        print(f"\n===== FedDyn + CKKS Profiled Round {round_idx}/{GLOBAL_ROUNDS} =====")
        ...
        for client_idx, client_dir in enumerate(client_dirs):
            ...
            # CKKS encryption is used here for secure transmission profiling.
            # Final FedDyn aggregation remains mathematically unchanged.
        ...
        # We perform encrypted aggregation/decryption only to measure HE overhead.
        # The decrypted output is not injected into FedDyn because FedDyn has its own dynamic correction.
        ...
        row = {
            ...
            "client_sizes": client_sizes,
            "alpha": ALPHA,
            "ckks_usage": "secure transmission and overhead profiling",
            "aggregation_note": "FedDyn aggregation is kept unchanged; CKKS classifier aggregation is profiled but not injected.",
    ...
    final_results = {
        "experiment": "EXP-007B",
        "method": "FedDyn + Selective CKKS Profiling using TenSEAL",
    ...
    with open(RESULTS_DIR / "covid_feddyn_ckks_profiled.json", "w") as f:
        json.dump(final_results, f, indent=4)

    torch.save(
        global_model.state_dict(),
        RESULTS_DIR / "covid_feddyn_ckks_profiled.pth",
    )

    print("\nSaved FedDyn + CKKS profiled results to:", RESULTS_DIR)
