...
RESULTS_DIR = Path("results/multikrum_adaptive_ckks_budget_sweep")
...
def run_budget(max_selected_bytes):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("\n\n================================================")
    print(f"Running EXP-007G Budget Sweep | Budget={max_selected_bytes} bytes")
    print("================================================")
    print("Device:", device)
    ...
    for round_idx in range(1, GLOBAL_ROUNDS + 1):
        print(
            f"\n===== Budget Sweep Multi-Krum + Adaptive CKKS "
            f"Round {round_idx}/{GLOBAL_ROUNDS} | Budget={max_selected_bytes} ====="
        )
        ...
        print(f"Selected {len(selected_keys)} adaptive CKKS keys")
        ...
        row = {
            ...
            "selected_keys": selected_keys,
            "ckks_usage": "budget-sweep adaptive secure transmission and overhead profiling",
            "aggregation_note": "Multi-Krum aggregation is kept unchanged; adaptive CKKS is used for privacy/overhead profiling.",
            ...
            # Paper-friendly aliases
            "avg_information_coverage_ratio": adaptive_metrics.get("avg_update_coverage_ratio", 0.0),
            "avg_residual_leakage_risk": adaptive_metrics.get("avg_information_leakage_ratio", 0.0),
            "avg_reconstruction_risk_score": adaptive_metrics.get("avg_reconstruction_risk_score", 0.0),
            "avg_parameter_encryption_ratio": adaptive_metrics.get("avg_parameter_encryption_ratio", 0.0),
            "avg_adaptive_encryption_quality": adaptive_metrics.get("avg_adaptive_encryption_quality", 0.0),
            **adaptive_metrics,
            **metrics,
        }
        ...
        print(
            ...
            f"ICR/UCR: {row['avg_information_coverage_ratio']:.4f} | "
            f"PER: {row['avg_parameter_encryption_ratio']:.6f} | "
            f"AEQ: {row['avg_adaptive_encryption_quality']:.2f} | "
            f"RLR/RRS: {row['avg_residual_leakage_risk']:.4f} | "
    ...
    budget_result = {
        "experiment": "EXP-007G",
        "method": "Adaptive CKKS Budget Sweep with Multi-Krum Byzantine Defense",
    ...
    out_file = RESULTS_DIR / f"budget_{max_selected_bytes}_results.json"
    with open(out_file, "w") as f:
        json.dump(budget_result, f, indent=4)

    torch.save(
        global_model.state_dict(),
        RESULTS_DIR / f"budget_{max_selected_bytes}_final_model.pth",
    )
...
def main():
    ...
    for result in all_results:
        ...
        summary_rows.append(
            {
                "budget": result["max_selected_bytes"],
                "final_accuracy": final["accuracy"],
                "final_f1": final["f1"],
                "best_accuracy": best["accuracy"],
                "best_f1": best["f1"],
                "final_icr": final["avg_information_coverage_ratio"],
                "final_rlr": final["avg_residual_leakage_risk"],
                "final_rrs": final["avg_reconstruction_risk_score"],
                "final_per": final["avg_parameter_encryption_ratio"],
                "final_aeq": final["avg_adaptive_encryption_quality"],
                "final_crypto_overhead_percent": final["crypto_overhead_percent"],
                "final_encrypted_upload_mb": final["selective_encrypted_upload_mb"],
                "final_round_time_sec": final["round_time_sec"],
            }
        )

    sweep_summary = {
        "experiment": "EXP-007G",
        "method": "Adaptive CKKS Budget Sweep",
        "budgets": BUDGETS,
        "top_k": TOP_K,
    ...
    for row in summary_rows:
        print(
            ...
            f"ICR={row['final_icr']:.4f} | "
            f"RLR={row['final_rlr']:.4f} | "
            f"PER={row['final_per']:.6f} | "
            f"AEQ={row['final_aeq']:.2f} | "
            f"crypto={row['final_crypto_overhead_percent']:.2f}% | "
            f"upload={row['final_encrypted_upload_mb']:.2f} MB"
        )
