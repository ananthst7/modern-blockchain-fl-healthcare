...
from federated.ila_client import train_ila_client
...
RESULTS_DIR = Path("results/multikrum_ila_ckks")
...
def main():
    ...
    for round_idx in range(1, GLOBAL_ROUNDS + 1):
        print(f"\n===== ILA-CKKS + Multi-Krum Round {round_idx}/{GLOBAL_ROUNDS} =====")
        ...
        for client_idx, client_dir in enumerate(client_dirs):
            state, size, loss, fisher_scores, gradvar_scores = train_ila_client(
        ...
        selected_keys, selected_plain_budget_bytes, ranked_ila_scores = (
            select_ila_keys_under_budget(
        ...
        ila_privacy_metrics = aggregate_ila_privacy_metrics(ila_privacy_metric_rows)
        independent_metric_rows = []

        for idx, state in enumerate(client_states):
            independent_metric_rows.append(
                compute_independent_coverage_metrics(
                    client_state=state,
                    global_state=global_state_cpu,
                    fisher_scores=fisher_scores_by_client[idx],
        ...
        independent_metrics = aggregate_independent_coverage_metrics(independent_metric_rows)
        ...
        pcr = ila_privacy_metrics.get("avg_privacy_coverage_ratio", 0.0)
        rpl = ila_privacy_metrics.get("avg_residual_privacy_leakage", 0.0)
        ...
        row = {
            ...
            "ila_privacy_metrics_per_client": ila_privacy_metric_rows,
            ...
            **adaptive_metrics,
            **ila_privacy_metrics,
            "independent_coverage_metrics_per_client": independent_metric_rows,
            ...
            "avg_gradient_cosine_similarity": independent_metrics.get("avg_gradient_cosine_similarity", 0.0),
            **independent_metrics,
            **metrics,
        }
        ...
        print(
            ...
            f"Infl: {independent_metrics.get('avg_influence_coverage_ratio', 0):.4f} | "
            f"Cos: {independent_metrics.get('avg_gradient_cosine_similarity', 0):.4f} | "
    ...
    final_results = {
        "experiment": "EXP-007I",
        "method": "Information Leakage-Aware Adaptive CKKS with Multi-Krum Byzantine Defense",
        ...
        "he_library": "TenSEAL",
        "he_scheme": "CKKS",
        "selection_strategy": "update_norm * fisher_score * gradient_variance",
        "max_selected_bytes": MAX_SELECTED_BYTES,
        "device": str(device),
    ...
    with open(RESULTS_DIR / "covid_multikrum_ila_ckks.json", "w") as f:
        json.dump(final_results, f, indent=4)

    torch.save(
        global_model.state_dict(),
        RESULTS_DIR / "covid_multikrum_ila_ckks.pth",
    )

    print("\nSaved ILA-CKKS results to:", RESULTS_DIR)
