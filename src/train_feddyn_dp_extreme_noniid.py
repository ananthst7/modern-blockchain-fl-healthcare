...
RESULTS_DIR = Path("results/feddyn_dp_extreme_noniid")
...
NOISE_STD = 1e-6
CLIP_NORM = 100.0
DP_TYPE = "classifier-only update-level DP"
...
def main():
    ...
    for round_idx in range(1, GLOBAL_ROUNDS + 1):
        print(f"\n===== FedDyn + DP Global Round {round_idx}/{GLOBAL_ROUNDS} =====")
    ...
    final_results = {
        "experiment": "EXP-006B",
        "method": "FedDyn + Approximate Update-Level DP + Extreme Non-IID",
        ...
        "dp_type": DP_TYPE,
        "formal_epsilon": None,
        "epsilon_note": "This update-level DP approximation does not provide a formal epsilon guarantee. Formal DP accounting requires per-sample gradients such as Opacus DP-SGD.",
        "device": str(device),
    ...
    with open(RESULTS_DIR / "covid_feddyn_dp_extreme_noniid.json", "w") as f:
        json.dump(final_results, f, indent=4)

    torch.save(
        global_model.state_dict(),
        RESULTS_DIR / "covid_feddyn_dp_extreme_noniid.pth",
    )

    print("\nSaved FedDyn + DP results to:", RESULTS_DIR)
